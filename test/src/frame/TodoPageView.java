...
public class TodoPageView extends JFrame {
	...
	public TodoPageView(LocalDate date, CalendarFrame01 mainFrame, ThemeManager themeManager) {
		...
		topSectionPanel.setBounds(10, 10, 445, 70);
		add(topSectionPanel);
			...
			if (!isDeleteMode) {
				...
				for (int i = 0; i < deleteCheckboxes.size(); i++) {
					if (deleteCheckboxes.get(i).isSelected()) {
						CalendarFrame01.TodoEntry todo = SessionManager.getCurrentUser().getDailyTasks()
								.getOrDefault(currentDate, new ArrayList<>()).get(i);
...
//	}
	...
	public void loadTodoList() {
		...
		List<CalendarFrame01.TodoEntry> todoList = SessionManager.getCurrentUser().getDailyTasks()
				.getOrDefault(currentDate, new ArrayList<>());
		...
		updateProgressBar();
		oneLineReviewField.setText(
				SessionManager.getCurrentUser().getDailyReviews().getOrDefault(currentDate, "예시: 오늘 하루도 멋지게 완수!"));
	...
	private void openModifyWindow(String uuid) {
		...
		if (foundIndex != -1) {
			new todoModify(SessionManager.getCurrentUser().getTodolist(), foundIndex, () -> {
				...
				java.util.Map<LocalDate, List<CalendarFrame01.TodoEntry>> dailyTasks = SessionManager.getCurrentUser()
						.getDailyTasks();
				...
				if (newDate != null) {
					if (existingEntry == null) {
						CalendarFrame01.TodoEntry newEntry = new CalendarFrame01.TodoEntry(updatedItem.getId(),
								updatedItem.getWork(), false, new java.awt.Color(255, 255, 204));
