...
//		setBackground(Color.black);
		...
		// 네모난 기본 배경 칠하지 않도록
		scrollPane.putClientProperty("excludeTheme", Boolean.FALSE);
		scrollPane.putClientProperty("roundPanel", Boolean.TRUE);
		ThemeManager.register("groupA", scrollPane);
			...
			try {
				...
				if (item.getGroup().equals(category) && itemYM.equals(ym)) {
					JLabel todoLabel = new JLabel(
							" " + item.getWork() + " | " + item.getDay() + " | " + item.getTime());
